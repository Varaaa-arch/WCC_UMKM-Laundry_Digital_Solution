    ...
    <div className="min-h-screen flex items-center justify-center bg-blue-50 p-4">
      <div className="w-full max-w-4xl flex rounded-3xl overflow-hidden shadow-2xl">
        ...
        <div className="w-full md:w-1/2 bg-white flex flex-col justify-center px-10 py-12">
          ...
          {/* Avatar */}
          <motion.div custom={0} variants={item(0)} initial="hidden" animate="visible" className="flex justify-center mb-6">
          ...
          <form onSubmit={handleLogin} className="space-y-5">
            ...
            {/* Remember + Forgot */}
            <motion.div variants={item(4)} initial="hidden" animate="visible" className="flex items-center justify-between">
              <label className="flex items-center gap-2 cursor-pointer select-none">
                <input type="checkbox" checked={rememberMe} onChange={e => setRememberMe(e.target.checked)}
                  className="w-4 h-4 rounded border-gray-300 accent-blue-600" />
                <span className="text-sm text-gray-600">Remember me</span>
              </label>
              <button type="button" className="text-sm text-blue-500 hover:text-blue-700 transition-colors">
                Forgot password?
              </button>
            </motion.div>
          ...
          {/* Divider */}
          <motion.div variants={item(6)} initial="hidden" animate="visible" className="my-6 flex items-center gap-3">
          ...
          {/* Social */}
          <motion.div variants={item(7)} initial="hidden" animate="visible" className="flex gap-3">
            {[
          ...
          {/* Footer */}
          <motion.p variants={item(8)} initial="hidden" animate="visible" className="text-center text-sm text-gray-500 mt-6">
            Belum punya akun?{' '}
