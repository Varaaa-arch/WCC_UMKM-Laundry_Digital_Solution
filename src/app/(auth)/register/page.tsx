...
const DiscordIcon = () => (
  <svg className="w-5 h-5 shrink-0 text-[#5865F2]" fill="currentColor" viewBox="0 0 24 24">
...
export default function RegisterPage() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-[#EEF4FB] p-4 pt-28">
      <div className="w-full max-w-sm bg-white rounded-2xl shadow-lg p-8">
        ...
        <form onSubmit={handleRegister} className="space-y-4">
          <div>
            ...
            <input
              name="name" type="text" required placeholder="Masukkan nama lengkapmu"
          ...
          <div>
            ...
            <div className="relative">
              <input
                name="password" type={showPassword ? "text" : "password"} required placeholder="Buat password"
          ...
          <div>
            ...
            <div className="relative">
              <input
                type={showConfirm ? "text" : "password"} required placeholder="Ulangi password"
                value={confirm} onChange={e => setConfirm(e.target.value)}
                className={`w-full px-4 py-3 pr-11 border rounded-xl text-sm outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all ${confirm && password !== confirm ? 'border-red-400' : 'border-gray-200'}`}
              />
            ...
            {confirm && password !== confirm && <p className="text-xs text-red-500 mt-1">Password tidak cocok</p>}
          </div>

          {error && <p className="text-sm text-red-500">{error}</p>}

          <button
            type="submit" disabled={isPending || (!!confirm && password !== confirm)}
          ...
          >
            {isPending ? <Loader2 className="w-4 h-4 animate-spin" /> : "Buat akun"}
          </button>
        </form>

        <p className="text-center text-sm text-gray-500 mt-6">
          Udah punya akun?{' '}
          <a href="/login" className="text-blue-500 font-semibold hover:underline">Masuk yuk</a>
        </p>
      </div>
    </div>
  )
